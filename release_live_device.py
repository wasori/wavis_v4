...
def main() -> None:
    try:
        ...
        print_header("WAVIS v4 활성 실거래 장치 해제")
        ...
        if engine_lock is None:
            print("\n실행 결과")
            print("engine_lock.json 파일이 아직 없습니다.")
            print("먼저 init_engine_lock.py 를 실행해라.")
            return
        ...
        if active_live_device is None:
            ...
            engine_lock["last_action"] = "release_live_device_skipped"
            ...
            engine_lock["message"] = "현재 해제할 활성 실거래 장치가 없습니다."

            save_json_file(engine_lock_path, engine_lock)

            print("\n실행 결과")
            print("현재 해제할 활성 실거래 장치가 없습니다.")
            return

        if active_live_device != device_id:
            engine_lock["last_updated_at"] = now_str
            engine_lock["last_updated_by"] = device_id
            engine_lock["last_action"] = "release_live_device_denied"
            engine_lock["current_snapshot"] = {
                "device_id": device_id,
                "app_mode": app_mode,
                "machine_name": machine_name,
            ...
            engine_lock["live_order_allowed_for_current_device"] = False
            engine_lock["message"] = (
                f"현재 장치({device_id})는 활성 실거래 장치({active_live_device})가 아니므로 "
                "해제할 수 없습니다."
            )
            ...
            print("\n실행 결과")
            print(
                f"현재 장치({device_id})는 활성 실거래 장치({active_live_device})가 아니므로 해제할 수 없습니다."
            )
            return

        engine_lock["active_live_device"] = None
        engine_lock["active_live_mode"] = None
        engine_lock["active_since"] = None
        engine_lock["last_updated_at"] = now_str
        engine_lock["last_updated_by"] = device_id
        engine_lock["last_action"] = "release_live_device"
        engine_lock["current_snapshot"] = {
            "device_id": device_id,
            "app_mode": app_mode,
            "machine_name": machine_name,
        ...
        engine_lock["live_order_allowed_for_current_device"] = False
        engine_lock["message"] = "활성 실거래 장치가 해제되었습니다."

        save_json_file(engine_lock_path, engine_lock)

        print("\n실행 결과")
        print("활성 실거래 장치가 해제되었습니다.")
