...
def evaluate_live_order_guard() -> dict[str, Any]:
    ...
    if app_mode != "live":
        result["reason"] = "APP_MODE가 live가 아니므로 테스트 주문 실행이 차단됩니다."
        return result

    if engine_lock is None:
        result["reason"] = "engine_lock.json 파일이 없어 테스트 주문 실행이 차단됩니다."
        return result

    if not bool(engine_lock.get("lock_enabled", True)):
        result["reason"] = "engine lock이 비활성화되어 있어 테스트 주문 실행이 차단됩니다."
        return result

    active_live_device = engine_lock.get("active_live_device")
    if active_live_device is None:
        result["reason"] = "활성 실거래 장치가 지정되지 않아 테스트 주문 실행이 차단됩니다."
        return result
...
def build_test_market_buy_payload(market: str, order_krw: float) -> dict[str, str]:
    identifier = (
        f"wavis-{get_env_str('DEVICE_ID', 'unknown')}-"
...
def request_test_order(payload: dict[str, str]) -> dict[str, Any]:
    headers = make_auth_headers(body_params=payload)

    response = requests.post(
        f"{BASE_URL}/v1/orders/test",
        headers=headers,
        json=payload,
        timeout=10,
    )
    ...
    if not isinstance(data, dict):
        raise ValueError("테스트 주문 응답 형식이 올바르지 않습니다.")
...
def main() -> None:
    try:
        ...
        print_header("WAVIS v4 시장가 매수 테스트 주문")
        ...
        if target_market is None:
            print_header("최종 결과")
            print("현재 진입 후보가 없어 테스트 주문을 만들지 않습니다.")
            sys.exit(1)

        payload = build_test_market_buy_payload(
        ...
        print_header("업비트 테스트 주문 요청")
        print(json.dumps(payload, ensure_ascii=False, indent=2))

        response_data = request_test_order(payload)

        print_header("업비트 테스트 주문 응답")
        print(json.dumps(response_data, ensure_ascii=False, indent=2))

        print_header("최종 결과")
        print("실제 체결 없이 테스트 주문 검증이 완료되었습니다.")
        print("다음 단계에서 같은 구조를 실제 주문 파일로 연결할 수 있습니다.")
